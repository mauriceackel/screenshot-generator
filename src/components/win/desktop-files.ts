...
type FileConfig = {
  ...
  highlightState: number; // 0 = none, 1 = active selection, 2 = hover selection
};
...
export default class DesktopFiles extends Component {
  ...
  public override async loadResources(): Promise<void> {
    ...
    const fileIconsPath = `${RESOURCE_PATH}/win/fileicons`;
        ...
        case 0:
          {
            ...
            ctx.fillText(
              ...
              fileConfig.boundingBox.y + fileConfig.iconBoundingBox.height,
        ...
        case 1:
          {
            ...
            ctx.rect(fileConfig.boundingBox.x - 4, fileConfig.boundingBox.y - 2, fileConfig.boundingBox.width + 8, fileConfig.boundingBox.height + 4);
            ctx.fill();
            ctx.stroke();
  ...
  public override getDrawJobs(config: DrawConfig): DrawJob[] {
    ...
    for (let i = 0; i < fileCount; i++) {
      ...
      const boundingBox: Rectangle = { x: totalX, y: iconY, width: fullWidth, height: iconBoundingBox.height + textHeight };
